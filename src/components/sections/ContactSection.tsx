      ...
      <div className="contact-panels">
        <div className="contact-directory">
          ...
          {contacts.filter((contact) => contact.label in contactIcons).map((contact) => {
            ...
            return contact.href ? <a className="contact-item" href={contact.href} key={contact.label} rel={contact.external ? "noreferrer" : undefined} target={contact.external ? "_blank" : undefined}>{content}</a> : <span className="contact-item is-disabled" key={contact.label}>{content}</span>;
          })}
        </div>
